          ...
          return (
            <button
              ...
              className={`rounded-full border px-3 py-1.5 text-sm transition dark:text-slate-200 ${
                active ? 'border-amber-500 bg-amber-100 dark:bg-amber-900/40' : 'border-slate-300 hover:border-amber-400 dark:border-slate-600'
              }`}
            >
              {it.label}
            </button>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        {buckets.map((b) => (
          <button
            key={b}
            onClick={() => place(b)}
            className="min-h-[88px] rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 p-3 text-left transition hover:border-amber-400 dark:border-slate-600 dark:bg-slate-900/40"
          >
