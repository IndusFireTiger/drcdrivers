...
export default function LessonView({ lesson, nextSlug }) {
  ...
  return (
    <article className="mx-auto max-w-2xl space-y-10 px-5 py-10">
      <header>
        <a href="/" className="text-sm text-slate-500 hover:text-amber-600">← The road</a>
        <h1 className="mt-2 text-3xl font-bold text-slate-900">
          <span className="mr-2">{lesson.icon}</span>Stop {lesson.id}: {lesson.title}
        </h1>
        <p className="mt-1 text-slate-500">{lesson.subtitle}</p>
      </header>

      {/* Hook */}
      <section className="space-y-3">
        <p className="text-slate-700">{lesson.hook.metaphor}</p>
        <blockquote className="border-l-4 border-amber-400 bg-amber-50 px-4 py-3 text-slate-700">
      ...
      <section className="space-y-4">
        <h2 className="text-lg font-semibold text-slate-800">Learn</h2>
        {lesson.learn.map((p, i) => (
          <p key={i} className="text-slate-700" dangerouslySetInnerHTML={{ __html: mdBold(p) }} />
        ))}
      </section>

      {/* Build bridge */}
      <section className="rounded-xl bg-slate-900 px-5 py-4 text-slate-100">
      ...
      <section className="space-y-5">
        <h2 className="text-lg font-semibold text-slate-800">Try it</h2>
      ...
      <section className="rounded-xl border border-amber-200 bg-amber-50 px-5 py-4">
        <span className="text-xs font-semibold uppercase tracking-wide text-amber-700">Teach it back</span>
        <p className="mt-1 text-slate-700">{lesson.teachBack}</p>
      </section>

      {/* Checkpoint */}
      <section className="space-y-4">
        <h2 className="text-lg font-semibold text-slate-800">Checkpoint</h2>
      ...
      <footer className="border-t border-slate-200 pt-6">
        {passed && nextSlug ? (
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <a href="/" className="inline-block rounded-lg border border-slate-300 px-5 py-3 font-medium text-slate-600 hover:border-amber-400">
              Back to the map
            </a>
          </div>
        ) : (
